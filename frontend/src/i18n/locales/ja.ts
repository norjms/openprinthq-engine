export default {
  ...
  printers: {
    ...
    modal: {
      ...
      typeMks: 'MKS',
      typeObico: 'Obico',
      obicoApiKey: 'Obico APIキー',
      obicoApiKeyHelp: 'Obicoアカウント設定（設定 → APIキー）から取得します。必須です。',
      obicoPrinterId: 'Obicoプリンター ID',
      obicoPrinterIdHelp: 'Obicoアカウント内でのプリンターID（Bambuddyのプリンター IDではありません）。Obicoのプリンター一覧で確認できます。',
      hostPort: 'ポート',
      apiKeyOptional: 'API キー（任意）',
      apiKeyHelp: 'プリンターが API キー／トークンを必要とする場合のみ入力します。',
